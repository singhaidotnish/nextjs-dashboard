...
import { recordFrameworkVersion, updateBuildDiagnostics } from '../diagnostics/build-diagnostics';
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, experimentalBuildMode, traceUploadUrl) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            await recordFrameworkVersion("15.0.0-canary.56");
            ...
            const providedPagePaths = JSON.parse(process.env.NEXT_PROVIDED_PAGE_PATHS || '[]');
            let pagesPaths = providedPagePaths.length > 0 ? providedPagePaths : !appDirOnly && pagesDir ? await nextBuildSpan.traceChild('collect-pages').traceAsyncFn(()=>recursiveReadDir(pagesDir, {
            ...
            if (appDir) {
                const providedAppPaths = JSON.parse(process.env.NEXT_PROVIDED_APP_PATHS || '[]');
                let appPaths = providedAppPaths.length > 0 ? providedAppPaths : await nextBuildSpan.traceChild('collect-app-paths').traceAsyncFn(()=>recursiveReadDir(appDir, {
